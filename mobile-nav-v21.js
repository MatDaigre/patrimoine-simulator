(() => {
  ...
  const ready = () => {
    if (document.body.dataset.mobileNavReady === '21') return;
    ...
    document.body.dataset.mobileNavReady = '21';
    document.body.classList.add('mobile-app-ready', 'mobile-app-v21');
    ...
    const goalsCard = sideChildren.find((node) => node.querySelector?.('h3')?.textContent?.trim() === 'Objectifs long terme');

    const views = {
      home: [hero, stats, journeyCard, healthCard].filter(Boolean),
      budget: [budget].filter(Boolean),
      invest: [invest, assetCard].filter(Boolean),
      projects: [projectsGroup, loan, realEstate].filter(Boolean),
      bilan: [recapCard, goalsCard, debtCard, journeyCard, historyCard, eventCard, disclaimerCard].filter(Boolean),
    };

    const controlled = new Set([hero, stats, ...mainChildren, ...sideChildren].filter(Boolean));
    controlled.forEach((node) => node.dataset.mobileManaged = '1');
    ...
    nav.className = 'mobile-bottom-nav mobile-bottom-nav-v21';
